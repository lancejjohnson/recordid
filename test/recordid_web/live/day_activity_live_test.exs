defmodule RecordidWeb.DayActivityLiveTest do
  ...
  test "editing an activity", %{conn: conn, user: user} do
    day = ~D[2023-03-15]
  ...
  test "deleting an activity", %{conn: conn, user: user} do
    day = ~D[2023-03-15]
    activities = create_activities_for_day(day, user, 3)
    {:ok, live_view, _html} = live(conn, ~p"/days/#{Date.to_string(day)}")

    activity = Enum.random(activities)

    live_view
    |> element("a", activity.description)
    |> render_click()

    assert_patch(live_view, ~p"/days/#{Date.to_string(day)}/activities/#{activity.id}/edit")
  ...
  defp create_activities_for_day(day, user, count) do
    for n <- 0..count do
      activity_fixture(
        user_id: user.id,
        description: "Test activity #{n} for day #{day} and user #{user.id}",
        time_started: ~T[00:01:00],
        time_finished: Time.add(~T[00:01:00], 25, :minute),
        date_started: day,
        date_finished: day
